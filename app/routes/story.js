...
function scheduleCronJob() {
  // Start the job every 4 hours
  job = cron.schedule('0 */4 * * *', runJob);
  console.log("Cron job scheduled to start and end every 4 hours.");
}
...
function runJob() {
  ...
  try {
    ...
    // Restart the job after a 4-hour pause
    setTimeout(() => {
      console.log("Restarting cron job...");
      scheduleCronJob();
    }, 4 * 3600000); // 4 hours in milliseconds
    return;
  }
  ...
  }, 60 * 60000); // 30 minutes in milliseconds
}
